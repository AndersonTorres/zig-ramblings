# -*- mode: org; coding: utf-8-unix; fill-column: 80 -*

* Capítulo 1

  Este capítulo dá uma visão geral da linguagem Zig, mediante exemplos
  práticos. Exibiremos e analisaremos diversos códigos completos a fim de
  ilustrar os conceitos da linguagem. Evitamos adentrar em detalhes técnicos
  neste capítulo, dado que teremos espaço para isso nos capítulos futuros.

** Considerações Gerais

    De maneira geral, um computador contém uma coleção de programas e arquivos
    especiais, os quais formam aquilo que chamamos de /sistema operacional/.
    Existe uma gama bastante variada de sistemas operacionais, indo desde os
    mais conhecidos comercialmente Windows e Mac OS X até outros com maior apelo
    técnico no mundo /open source/ como as distribuições Linux, os descendentes
    do BSD (FreeBSD, NetBSD, OpenBSD) e até mesmo projetos de reavivamento como
    o FreeDOS e o Haiku OS, este último inspirado no extinto BeOS.

    Um sistema operacional tem por objetivo gerenciar os recursos da máquina e
    fornecer interfaces convenientes entre estes recursos e os seus usuários.

    Entre os diversos recursos fornecidos pelo sistema operacional, estão
    editores de texto e interpretadores de linguagens de programação. Comumente
    estes editores e interpretadores são distribuídos de forma independente
    entre si; porém, não é incomum a presença de ambientes integrados de
    programação. De fato, existem pacotes de integração que costumam ser mais
    famosos que seus próprios componentes individuais, como as IDEs
    comercializadas pela Borland e Microsoft.

    Por ora, vamos assumir que o leitor consiga criar e editar arquivos de texto
    contendo código Zig, além de acesso a uma shell de comandos e do compilador
    Zig oficial. Mais adiante, vamos comentar o processo de obter um ambiente
    apropriado para esta tarefa.

** Conceitos

*** Um Programa Simples

    Vamos começar com um programa simples que exibe uma frase na tela do
    terminal.

    Escreva o texto abaixo em um arquivo com o nome terminado em =.zig=, digamos
    =hello.zig=.

    #+begin_src zig :mkdirp yes :tangle ./code/chapter-01/hello.zig
      const std = @import("std");
      const dprint = std.debug.print;

      pub fn main() void {
          dprint("{s}", .{"Hello, World!\n"});
      }
    #+end_src

**** Execução

     Agora, para compilar este programa, execute o comando =zig build-exe
     hello.zig=. Se o código não contiver erros que impeçam uma compilação
     bem-sucedida, um arquivo de mesmo nome mas com extensão diferente será
     criado como resultado, no caso, =hello=. Podemos então executá-lo:

     #+begin_example
     $> zig build-exe hello.zig
     $> ./hello
     Hello, World!
     $>
     #+end_example

**** Destrinchando

    Vamos então percorrer este programa linha a linha:

    #+begin_src zig
      const std = @import("std");
    #+end_src

    Esta é uma típica atribuição. Vamos destrinchá-la:

    - =const= é uma palavra reservada[fn:1] da linguagem Zig; ela tem um
      propósito distinto, e é tratada de uma maneira especial pelo compilador.
      Especificamente, ela indica que o identificador =std= é uma constante,
      armazenando um valor imutável.

    - =std= é um identificador. Identificadores servem para identificar,
      fornecer uma identidade a uma entidade utilizada no programa.

    - =@import( ... )= é uma função embutida na linguagem Zig. Esta função tem
      uma funcionalidade bastante importante: ela recebe como parâmetro a
      localização, no sistema de arquivos, onde se situa um arquivo escrito na
      linguagem Zig, e retorna uma estrutura correspondente a este
      arquivo. [fn:: Na linguagem Zig, um arquivo contendo código-fonte Zig é
      tratado como uma =struct=.].

      Normalmente, =@import= recebe como parâmetro o caminho relativo ao arquivo
      desejado; porém, há alguns valores especiais que este parâmetro pode
      assumir. Por exemplo, ="std"= representa a biblioteca-padrão de Zig.

   Sumarizando esta linha: =std= é uma constante que armazena a
   biblioteca-padrão de Zig.

    #+begin_src zig
      const dprint = std.debug.print;
    #+end_src

    Da biblioteca =std=, vamos utilizar uma função para exibir frases na tela, a
    saber, =std.debug.print=. Como o nome sugere, ela costuma ser utilizada para
    "debug print", uma antiga técnica de depuração que consiste em emitir
    mensagens na tela a fim de rastrear o comportamento de um programa.

    Para escrever menos, atribuímos a ela o identificador =dprint=.[fn:: De
    fato, poderíamos ter escrito =@import("std").debug.print( ... )=
    diretamente, todavia isto sacrificaria muito da legibilidade do programa.]

    #+begin_src zig
      pub fn main() void {
    #+end_src

    Esta é uma declaração de função. Vamos destrinchá-la:

    - A palavra-chave =pub= indica que o objeto definido (a saber, =main=) será
      /público/. Isto significa que ele estará visível acaso algum código Zig a
      importe.

    - A palavra-chave =fn= indica que o objeto definido - a saber, =main= -
      trata-se de uma função.

      A função =main= tem um propósito especial em Zig. Ela indica ao compilador
      onde o programa inicia sua execução (no jargão, =main= é o /ponto de
      entrada/ do programa). Normalmente, programas feitos para serem executados
      dependem de uma função =main= que seja pública.[fn:2]

    - Por fim, =void= é mais uma palavra reservada; especificamente aqui, ela
      indica que a função =main()= retorna =void= (ou seja, ela não retorna
      valor algum para quem a chamou).

    - As chaves ={ ... }= envolvem o corpo de uma definição de função. Mais
      geralmente, chaves servem para agrupar comandos e outros tipos de
      declaração.

    #+begin_src zig
          dprint("{s}", .{"Hello, World!\n"});
    #+end_src

    Aqui estamos de fato empregando a função =dprint()=. Não adentraremos em
    detalhes de seu funcionamento neste momento, mas é possível falar do básico:

    Esta função recebe dois parâmetros. O primeiro, ="{s}"=, é uma string de
    formatação, enquanto o segundo, =.{"Hello, World!\n"}= é uma struct anônima
    que serve de parâmetro para a string de formatação.

*** Variáveis, Expressões e Atribuições

    Como ilustração, vamos escrever um programa que converte escalas de
    temperatura.

    #+begin_src zig :mkdirp yes :tangle ./code/chapter-01/temperature.zig
      const std = @import("std");
      const dprint = std.debug.print;

      pub fn main() void {
          var celsius_temperature: f32 = 100;
          var kelvin_temperature: f32 = undefined;
          var fahrenheit_temperature: f32 = undefined;

          // celsius to kelvin conversion
          kelvin_temperature = celsius_temperature + 273.16;
          fahrenheit_temperature = 9*(kelvin_temperature - 273.16)/5 + 32; // kelvin to fahrenheit conversion

          const format = "\tCelsius: {d:.3}\n\tFahrenheit: {d:.3}\n\tKelvin: {d:.3}\n";

          dprint(format, .{celsius_temperature,
                           fahrenheit_temperature,
                           kelvin_temperature});
      }
    #+end_src

**** Execução

     Outra forma de executar programas Zig é invocando o comando =zig run=. Este
     comando não gera um arquivo executável; em vez disso, ele executa o programa
     diretamente. [fn:: Veteranos acostumados com linguagens interpretadas como
     Python reconhecerão a semelhança!]

     #+begin_example
     $> zig run temperature.zig
         Celsius: 100.000
         Fahrenheit: 212.000
         Kelvin: 373.160
     $>
     #+end_example

**** Destrinchando

    #+begin_src zig
          var celsius_temperature: f32 = 100;
    #+end_src

    Esta é uma declaração de variável, como indicado pela palavra-chave =var=.
    Em geral, declarações e expressões em geral finalizam com um ponto-e-vírgula
    =;=.

    A palavra-chave =f32= indica que o valor armazenado pelo identificador
    =celsius_temperature= será do tipo =f32= - um ponto flutuante de 32
    bits. Inicializamos este valor com =100=.

    #+begin_src zig
          var kelvin_temperature: f32 = undefined;
          var fahrenheit_temperature: f32 = undefined;
    #+end_src

    Em Zig, é obrigatório inicializar as variáveis com algum valor. Caso não
    seja possível ou viável inicializar alguma variável com um valor concreto,
    usa-se para tal a palavra-chave =undefined= em seu lugar.

    #+begin_src zig
          // celsius to kelvin conversion
    #+end_src

    Esta linha é um comentário. Em Zig, comentários iniciam com =//= e encerram
    no fim da linha (LF - o caractere /linefeed/). O compilador ignora os
    comentários, não os transformando em código executável.

    Zig não tem o conceito de "comentário multi-linha" (como, por exemplo,
    comentários =/* ... */= em C++).

    #+begin_src zig
          kelvin_temperature = celsius_temperature + 273.16;
    #+end_src

    Esta é uma atribuição, como indicada pelo sinal ===. O valor da expressão do
    lado direito é atribuído à variável do lado esquerdo. Como já é esperado, as
    operações matemáticas seguem as regras de precedência e associatividade com
    as quais estamos acostumados: multiplicações e divisões são efetuadas antes
    de adições e subtrações.[fn:3]

    #+begin_src zig
          fahrenheit_temperature = 9*(kelvin_temperature - 273.16)/5 + 32; // kelvin to fahrenheit conversion
    #+end_src

    Como antes, esta é uma atribuição. Operações contidas entre parênteses são
    efetuadas primeiro.

    A título de demonstração, há um comentário após esta atribuição; comentários
    podem começar em qualquer posição de uma linha, estendendo-se até o fim da
    linha, como de costume.

    #+begin_src zig
          const format = "\tCelsius: {d:.3}\n\tFahrenheit: {d:.3}\n\tKelvin: {d:.3}\n";
    #+end_src

    Nesta linha, é atribuída a =format= uma cadeia de caracteres (vulgarmente
    conhecida como /string/). Esta string tem algumas particularidades que valem
    ser mencionadas.

    A primeira: as sequências de caracteres =\t= e =\n= são chamados /caracteres
    de escape/, e têm um significado especial. =\t= é o caractere de tabulação
    (TAB), e o caractere =\n= é o caractere que indica o início de uma nova
    linha.

    A segunda: os gabaritos de formatação ={d:.3}=. Eles são interpretados pela
    função =dprint=, e servem para determinar como os demais parâmetros serão
    impressos. Neste caso específico, serão numerais (=d=) impressos com 3 casas
    decimais após o ponto (=.3=).

    #+begin_src zig
          dprint(format, .{celsius_temperature,
                           fahrenheit_temperature,
                           kelvin_temperature});
      }
    #+end_src

    Esta é a função =dprint()=. O primeiro parâmetro já foi explicado antes. O
    segundo, como já foi dito, representa uma struct anônima com três campos, os
    quais serão relacionados com os gabaritos ={d:.3}= de =format=.

*** Estruturas de Controle

    Via de regra, os comandos de um programa são executados sequencialmente.
    Porém, muito comumente os programas exigem alteração neste fluxo sequencial
    padrão. As formas mais corriqueiras de desvio de fluxo envolvem repetições e
    ações baseadas em alternativas.

    Comandos =if-else= fornecem ações alternativas, enquanto comandos =while=
    fornecem mecanismos de repetição. Estas construções exigem a evaluação de
    expressões lógicas. Ao contrário de grande parte das linguagens de
    programação, em Zig somente expressões explicitamente booleanas são
    verdadeiras ou falsas. Em Zig, não se aplica o conceito onde "=NULL=
    significa falso, não-=NULL= significa verdadeiro".

    Vamos ilustrar com um pequeno programa que calcula as somas dos números
    naturais pares e dos ímpares entre um e mil (inclusive).

    #+begin_src zig :mkdirp yes :tangle ./code/chapter-01/sums.zig
      const std = @import("std");
      const dprint = std.debug.print;

      pub fn main() void {
          const lower_bound = 0;
          const upper_bound = 1_000;
          var i: i32 = lower_bound;
          var odd_sum: i32 = 0;
          var even_sum: i32 = 0;
          var all_sum: i32 = 0;

          while (i <= upper_bound) {
              all_sum += i;
              if (@mod(i, 2) == 1) {
                  odd_sum += i;
              } else {
                  even_sum += i;
              }
              i += 1;
          }

          const format =
              \\Lower bound: {d}
              \\Upper bound: {d}
              \\Sum of even numbers: {d}
              \\Sum of odd numbers: {d}
              \\Sum of all numbers: {d}
              \\
          ;

          dprint(format, .{lower_bound,
                           upper_bound,
                           even_sum,
                           odd_sum,
                           all_sum});
      }
    #+end_src

**** Execução

     #+begin_example
     $> zig run sums.zig
     Lower bound: 0
     Upper bound: 1000
     Sum of even numbers: 250500
     Sum of odd numbers: 250000
     Sum of all numbers: 500500
     $>
     #+end_example

**** Destrinchando

     #+begin_src zig
           const upper_bound = 1_000;
     #+end_src

     Em Zig, é possível escrever numerais extensos usando o caractere
     /underscore/ =_=. Ele não altera o valor representado pelo numeral,
     servindo primariamente para melhor legibilidade de números grandes. Esta
     linha, por exemplo, atribui o valor =1000= à constante =upper_bound=.

     #+begin_src zig
           while (i <= upper_bound) {
     #+end_src

     O comando =while= aqui é composto de uma condição de teste e de um bloco de
     comandos envoltos por chaves ={...}=. Em um comando =while= típico,
     ocorre o seguinte:

     1. A condição de teste, contida entre parênteses, é evaluada.
     2. Caso seja verdadeira, o bloco de comandos é executado, e o fluxo de
        controle retornará ao início do bloco, a fim de que a condição seja
        testada novamente.
     3.  Caso seja falsa, o fluxo de controle é direcionado para o comando
        seguinte ao bloco, após a chave de fechamento =}= (o que, neste caso, é
        o =const format ...=).

     #+begin_src zig
               all_sum += i;
     #+end_src

     Este é um tipo novo de atribuição: o /incremento/.

     Este comando significa que o valor armazenado em =all_sum= receberá um
     incremento do valor de =i=. Um comando equivalente seria

     #+begin_src zig
       all_sum = all_sum + i;
     #+end_src

     Prossigamos...

     #+begin_src zig
               if (@mod(i, 2) == 1) {
                   odd_sum += i;
               } else {
                   even_sum += i;
               }
     #+end_src

     Aqui temos o nosso comando =if-else=. Ele é composto de uma condição e de
     dois blocos de comandos. Em um comando =if-else= típico, ocorre o seguinte:

     1. A condição, contida em parênteses, é evaluada.
     2. Caso a condição seja verdadeira, o primeiro bloco é executado;
     3. Caso contrário, o segundo bloco, situado após o =else=, será executado.
     4. Após isto, o fluxo de controle seguirá para o próximo comando, a saber:

     #+begin_src zig
               i += 1;
     #+end_src

     Como explicamos antes, o valor de =i= é incrementado de 1.

     #+begin_src zig
           }
     #+end_src

     Esta chave encerra o comando =while (...)=. Como explicado antes, o fluxo
     de controle volta para a expressão =while= a fim de a condição ser
     novamente evaluada.

     #+begin_src zig
           const format =
               \\Lower bound: {d}
               \\Upper bound: {d}
               \\Sum of even numbers: {d}
               \\Sum of odd numbers: {d}
               \\Sum of all numbers: {d}
           ;
     #+end_src

     Uma novidade: a /string literal multilinha/.

     Strings literais multilinha, como o nome sugere, podem estender-se por
     várias linhas, e além disso, não reconhecem os caracteres de escape como
     especias; sequências de caracteres como =\n= ou =\t= são interpretadas
     literalmente.

     Uma string literal multilinha inicia-se com a sequência =\\= e prossegue
     até o final da linha. A string assim formada não inclui o caractere de nova
     linha ao final; porém, se a próxima linha é iniciada com =\\=, então o
     caractere de nova linha é incluso, e assim a string continua.

     #+begin_src zig
           dprint(format, .{lower_bound,
                            upper_bound,
                            even_sum,
                            odd_sum,
                            all_sum});
       }
     #+end_src

     E a nossa costumeira função =dprint=. Como antes, os gabaritos ={d}= são
     interpretados como números, enquanto o segundo argumento serve como
     parâmetro para a string de formatação.

*** Funções

    /Função/, também conhecido como /procedimento/, /sub-rotina/ ou
    /subprograma/, é (grosso modo) um agrupamento de comandos que realizam uma
    determinada tarefa.

    Em geral, programas Zig contam com pelo menos uma função, a =main()= como já
    vimos. É a partir dela que o programa se inicia. As diversas funções de um
    programa podem ser invocadas a partir de =main()= e podem invocar umas às
    outras.

    Uma função em Zig tem o seguinte formato:

    #+begin_example
    fn name(params) returnType {...}
    #+end_example

    onde =returnType= é o tipo do valor retornado pela função, e =params= são as
    declarações das variáveis de entrada da função.

    Vamos ilustrar o conceito de funções com um pequeno programa que verifica se
    um determinado número inteiro positivo é primo ou não. [fn:4] Vamos
    aproveitar o ensejo e inserir algumas novidades.

    #+begin_src zig :mkdirp yes :tangle ./code/chapter-01/primality.zig
      const std = @import("std");

      const dprint = std.debug.print;
      const expect = std.testing.expect;

      fn smallestDivisor(n: i64) i64 {
          var found: bool = false; // was a factor found?
          var exausted: bool = false; // was the search space exausted?
          var d: i64 = 2; // possible divisor being tested

          // while no divisor was found on the possible divisor range
          while (!found and !exausted){
              if (d*d > n) { // exausted the space
                  exausted = true;
              } else if (@mod(n, d) == 0) { // divisor found
                  found = true;
              } else { // update the divisor
                  d += 1;
              }
          }

          return (if (found) d else n);
      }

      test "smallestDivisor tests" {
          try expect(smallestDivisor(55) == 5);
          try expect(smallestDivisor(5*16) == 2);
          try expect(smallestDivisor(5*7) + smallestDivisor(7*11) == 5+7);
      }

      fn isPrime (n: i64) bool {
          return (smallestDivisor(n) == n);
      }

      test "isPrime tests" {
          try expect(isPrime(2*3+1));
          try expect(!isPrime(11111_11111));
      }

      pub fn main() void {
          const format =
              \\Number: {d}
              \\Smallest factor: {d}
              \\
          ;

          const N = 2013;

          dprint(format, .{N,
                           smallestDivisor(N)});
      }
    #+end_src

**** Execução

     #+begin_example
     $> zig test primality.zig
     Test [1/2] test "smallestDivisor tests"...
     Test [2/2] test "isPrime tests"...

     All 2 tests passed.
     $> zig run primality.zig
     Number: 2013
     Smallest factor: 3
     $>
     #+end_example

**** Destrinchando

    #+begin_src zig
      const expect = std.testing.expect;
    #+end_src

    =expect(...)= é uma função da biblioteca padrão de Zig. Ela tem uma
    funcionalidade importante, intimamente relacionada a testes, a qual
    explicaremos mais tarde.

    #+begin_src zig
          // while no divisor was found on the possible divisor range
          while (!found and !exausted){
                  if (d*d > n) { // exausted the space
                  exausted = true;
              } else if (@mod(n, d) == 0) { // divisor found
                  found = true;
              } else { // update the divisor
                  d += 1;
              }
          }
    #+end_src

    O algoritmo mais simples e mais conhecido para testar a primalidade de um
    número consiste em, sucessivamente, verificar todos os naturais menores que
    o número de entrada, até encontrar algum que seja divisor ou até exauri-los
    sem nada encontrar. Além disso, uma otimização muito conhecida deste
    algoritmo consiste em testar somente até a raiz quadrada do número de
    entrada, em vez de testar absolutamente todo o intervalo.[fn:5]

    Para codificar este algoritmo, precisaremos dos seguintes elementos, pelo
    menos:

    - uma estrutura de repetição para percorrer o espaço amostral de divisores;
    - dois marcadores:
      - um que sinalize se encontramos um divisor;
      - um que sinalize se ultrapassamos o limite de busca.

    Em qualquer destes dois casos, a repetição deve ser interrompida.

    #+begin_src zig
          return (if (found) d else n);
      }
    #+end_src

    Uma novidade: comandos =if= também funcionam como expressões![fn:6] A
    expressão acima pode ser reescrita na forma equivalente abaixo:

    #+begin_src zig zig
          if (found) {
              return d;
          } else {
              return n;
          }
      }
    #+end_src

    Assim sendo, se o marcador =found= for verdadeiro, isto significa que
    encontramos um divisor não-trivial, o qual obviamente está em =d=; caso
    contrário, o número de entrada não tem divisores não-triviais, sendo
    portanto primo.

    #+begin_src zig
      test "smallestDivisor tests" {
    #+end_src

    Zig fornece suporte a testes como parte integrante da linguagem.

    Um teste consiste nestes três elementos:

    - A palavra-chave =test=;
    - Uma string usada como identificador do teste;
    - Um bloco de comandos a ser executado.

    Para executar os testes, basta utilizar o comando =zig test=, como mostrado
    anteriormente.

    #+begin_src zig
          try expect(smallestDivisor(55) == 5);
    #+end_src

    Esta expressão exibe diversas peculiaridades sobre a linguagem Zig. Neste
    capítulo introdutório, não vamos nos aprofundar nelas, teceremos apenas
    algumas considerações rápidas.

    - =try= é uma palavra reservada da linguagem Zig. Ela está relacionada ao
      sistema de tratamento de erros da linguagem. Grosso modo, um comando =try=
      executa o seu comando correspondente e, caso encontre um erro, ele retorna
      imediatamente da função com este erro, senão ele prossegue para o próximo
      comando

      Especificamente aqui, ele executa =expect(smallestDivisor(55) == 5)= e
      retorna em caso de erro ou prossegue caso contrário.

    - =expect(...)= é uma função da biblioteca padrão de Zig. Quando seu
      argumento é =false=, ela emite uma mensagem de erro e aborta a execução.

    #+begin_src zig
      fn isPrime (n: i64) bool {
          return (smallestDivisor(n) == n);
      }
    #+end_src

    Por "definição", um número primo tem ele mesmo como menor divisor (além
    do trivial 1).

    #+begin_src zig
      test "isPrime tests" {
          try expect(isPrime(2*3+1));
          try expect(!isPrime(11111_11111));
      }
    #+end_src

    Podemos ter mais de um bloco de testes em Zig.

    Como antes, escrevemos o número 1111111111 (dez dígitos 1) usando
    /underscores/. Note que não existe uma posição fixa para este caractere:
    anteriormente, usamos =1_000= como separador de milhares, enquanto aqui
    separamos em grupos de cinco. A única restrição é que o underscore não pode
    ser o primeiro caractere, porque com isso o "número" seria interpretado como
    um identificador.
